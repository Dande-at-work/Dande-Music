...
class DiscordRPC(
  ...
  companion object {
    ...
    private const val APP_ICON_URL = "https://avatars.githubusercontent.com/u/258176326?s=200&v=4"
  ...
  suspend fun closeRPC() {
    DiscordSocialPresenceClient.close().getOrElse { Timber.tag(TAG).v(it, "close failed") }
  }
  ...
  suspend fun refreshActivity(
    ...
    currentPlaybackTimeMillis: Long,
    isPaused: Boolean = false,
  ) = runCatching { updateSong(song, currentPlaybackTimeMillis, isPaused).getOrThrow() }
  ...
  private suspend fun resolveButtons(song: Song): List<DiscordPresenceButton> {
    ...
    val button2Label = context.dataStore[DiscordActivityButton2LabelKey] ?: "Go to Dande Music"
    ...
    val button2CustomUrl =
      context.dataStore[DiscordActivityButton2CustomUrlKey]
        ?: "https://github.com/Dande-at-work/Dande-Music"
    ...
    when (source.lowercase()) {
      ...
      "artisturl" -> {
        song.artists
          ...
          ?.id
          ?.takeUnless {
            song.song.isLocal || it.startsWith("LOCAL_ARTIST_") || it.isLocalMediaId()
          }
  ...
  private fun String?.toDiscordText(
    ...
    return value.takeIf { it.length >= 2 }?.take(maxLength)
  }
