...
interface DatabaseDao {
    ...
    when (sortType) {
      ...
      SongSortType.ARTIST ->
        songsByRowIdAsc().map { songs ->
          ...
          songs
            .sortedWith(
              compareBy(collator) { song -> song.artists.joinToString("") { it.name } },
            )
  ...
  @Transaction @Query("SELECT COUNT(1) FROM song WHERE liked") fun likedSongsCount(): Flow<Int>

  @Transaction
  @Query(
    "SELECT song.* FROM song JOIN song_album_map ON song.id = song_album_map.songId WHERE song_album_map.albumId = :albumId"
  )
  ...
  @Query(
    "SELECT SUM(playTime) FROM event WHERE timestamp >= :fromTimeStamp AND timestamp <= :toTimeStamp"
  )
  fun getTotalPlayTimeInRange(fromTimeStamp: Long, toTimeStamp: Long): Flow<Long?>

  @Query(
    "SELECT COUNT(DISTINCT songId) FROM event WHERE timestamp >= :fromTimeStamp AND timestamp <= :toTimeStamp"
  )
  ...
  @Transaction @Query("SELECT * FROM song") fun allSongs(): Flow<List<Song>>
  ...
  fun artists(sortType: ArtistSortType, descending: Boolean) =
    when (sortType) {
      ...
      artists.filter { it.artist.isYouTubeArtist || it.artist.isLocal }.reversed(descending)
    }
    ...
    when (sortType) {
      ...
      AlbumSortType.ARTIST ->
        albumsByCreateDateAsc().map { albums ->
          ...
          collator.strength = Collator.PRIMARY
          albums.sortedWith(
            compareBy(collator) { album -> album.artists.joinToString("") { it.name } }
          )
        }
  ...
  @Transaction
  @Query(
    "SELECT COUNT(*) from playlist_song_map WHERE playlistId = :playlistId AND songId = :songId LIMIT 1"
  )
  fun checkInPlaylist(
    playlistId: String,
    songId: String,
  ...
  @Query(
    "SELECT songId from playlist_song_map WHERE playlistId = :playlistId AND songId IN (:songIds)"
  )
  ...
  fun addSongToPlaylist(playlist: Playlist, songIds: List<String>) {
    ...
    songIds.forEach { id ->
      insert(PlaylistSongMap(songId = id, playlistId = playlist.id, position = position++))
    }
  }

  fun downloadedSongs(sortType: SongSortType, descending: Boolean): Flow<List<Song>> =
    when (sortType) {
      SongSortType.CREATE_DATE -> downloadedSongsByCreateDateAsc()
      SongSortType.NAME ->
        downloadedSongsByNameAsc().map { songs ->
          val collator = Collator.getInstance(Locale.getDefault())
          collator.strength = Collator.PRIMARY
          songs.sortedWith(compareBy(collator) { it.song.title })
        }
  ...
  @Transaction
  @Query(
    "SELECT * FROM song WHERE title LIKE '%' || :query || '%' AND inLibrary IS NOT NULL LIMIT :previewSize"
  )
  fun searchSongs(
    query: String,
  ...
  @Query("SELECT COUNT(*) FROM event") fun eventCount(): Flow<Int>

  @Transaction @Query("DELETE FROM event") fun clearListenHistory()
  ...
  @Transaction @Query("DELETE FROM search_history") fun clearSearchHistory()
  ...
  @Transaction
  @Query(
    "SELECT * FROM recognition_history WHERE title LIKE '%' || :query || '%' OR artist LIKE '%' || :query || '%' ORDER BY recognizedAt DESC"
  )
  fun searchRecognitionHistory(query: String): Flow<List<RecognitionHistory>>

  @Transaction @Query("DELETE FROM recognition_history") fun clearRecognitionHistory()
  ...
  @Delete fun delete(recognitionHistory: RecognitionHistory)
  ...
  @Transaction
  @Query(
    "UPDATE song SET libraryAddToken = :libraryAddToken, libraryRemoveToken = :libraryRemoveToken WHERE id = :songId"
  )
  ...
  @Query("SELECT * FROM artist WHERE id = :id LIMIT 1") fun getArtistById(id: String): ArtistEntity?

  @Insert(onConflict = OnConflictStrategy.IGNORE) fun insert(song: SongEntity): Long

  @Insert(onConflict = OnConflictStrategy.IGNORE) fun insert(artist: ArtistEntity)

  @Insert(onConflict = OnConflictStrategy.IGNORE) fun insert(album: AlbumEntity): Long

  @Insert(onConflict = OnConflictStrategy.IGNORE) fun insert(playlist: PlaylistEntity)

  @Insert(onConflict = OnConflictStrategy.IGNORE) fun insertInternal(map: SongArtistMap)
  ...
  @Insert(onConflict = OnConflictStrategy.IGNORE) fun insertInternal(map: SongAlbumMap)
  ...
  @Insert(onConflict = OnConflictStrategy.IGNORE) fun insertInternal(map: AlbumArtistMap)
  ...
  @Insert(onConflict = OnConflictStrategy.IGNORE) fun insertInternal(map: PlaylistSongMap)
  ...
  @Insert(onConflict = OnConflictStrategy.REPLACE) fun insert(searchHistory: SearchHistory)

  @Insert(onConflict = OnConflictStrategy.IGNORE) fun insert(event: Event)

  @Insert(onConflict = OnConflictStrategy.IGNORE) fun insertInternal(map: RelatedSongMap)
  ...
  @Insert(onConflict = OnConflictStrategy.IGNORE) fun insert(playCountEntity: PlayCountEntity): Long
  ...
  @Update fun update(song: SongEntity)

  @Update fun update(artist: ArtistEntity)

  @Update fun update(album: AlbumEntity)

  @Update fun update(playlist: PlaylistEntity)

  @Update fun update(map: PlaylistSongMap)
  ...
  fun update(playlistEntity: PlaylistEntity, playlistItem: PlaylistItem) {
    update(
      playlistEntity.copy(
        ...
        remoteSongCount =
          playlistItem.songCountText?.let { Regex("""\d+""").find(it)?.value?.toIntOrNull() },
  ...
  @Upsert fun upsert(map: SongAlbumMap)

  @Upsert fun upsert(lyrics: LyricsEntity)

  @Upsert fun upsert(format: FormatEntity)

  @Query("DELETE FROM format WHERE id = :id") fun deleteFormat(id: String)

  @Upsert fun upsert(beatInfo: BeatInfoEntity)

  @Query("SELECT * FROM beat_info WHERE songId = :songId")
  fun beatInfo(songId: String): BeatInfoEntity?

  @Upsert fun upsert(song: SongEntity)

  @Delete fun delete(song: SongEntity)

  @Delete fun delete(songArtistMap: SongArtistMap)

  @Delete fun delete(artist: ArtistEntity)

  @Delete fun delete(album: AlbumEntity)

  @Delete fun delete(albumArtistMap: AlbumArtistMap)

  @Delete fun delete(playlist: PlaylistEntity)

  @Delete fun delete(playlistSongMap: PlaylistSongMap)

  @Query("DELETE FROM playlist WHERE browseId = :browseId") fun deletePlaylistById(browseId: String)

  @Delete fun delete(lyrics: LyricsEntity)

  @Delete fun delete(searchHistory: SearchHistory)
  ...
  @Transaction
  @Query(
    "SELECT * FROM playlist_song_map WHERE playlistId = :playlistId AND position >= :from ORDER BY position"
  )
  ...
  @RawQuery fun raw(supportSQLiteQuery: SupportSQLiteQuery): Int
  ...
  @Query("SELECT id FROM song WHERE isLocal = 1") suspend fun localSongIds(): List<String>

  @Query("DELETE FROM song WHERE isLocal = 1") fun clearLocalSongs()
  ...
  @Query("DELETE FROM format WHERE id NOT IN (SELECT id FROM song)") fun pruneFormats()

  @Query("DELETE FROM playCount WHERE song NOT IN (SELECT id FROM song)") fun prunePlayCounts()

  @Query("DELETE FROM song WHERE id IN (:songIds)") fun deleteSongsByIds(songIds: List<String>)
